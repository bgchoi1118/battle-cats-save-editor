...
def get_event_stages_current() -> dict[str, Any]:
    ...
    clear_progress = list(chunks(clear_progress, stars_per_sub_chapter))
...
def get_event_stages(lengths: dict[str, Any]) -> dict[str, Any]:
    ...
    clear_progress = list(chunks(clear_progress, stars_per_sub_chapter))
    clear_amount = list(
        chunks(clear_amount, stages_per_sub_chapter * stars_per_sub_chapter)
    )
    unlock_next = list(chunks(unlock_next, stars_per_sub_chapter))
...
def get_ht_it_data() -> dict[str, Any]:
    ...
    current_data["selected"] = list(chunks(current_data["selected"], 4))
    ...
    progress_data["clear_progress"] = list(
        chunks(progress_data["clear_progress"], 4)
    )
    ...
    clear_amount = list(chunks(clear_amount, stages * stars))
...
def get_uncanny_current() -> dict[str, Any]:
    ...
    clear_progress = list(chunks(clear_progress, stars))
...
def get_event_timed_scores() -> dict[str, Any]:
    ...
    score = list(chunks(score, stars * stages_per_subchapter))
...
def get_uncanny_progress(lengths: dict[str, Any]) -> dict[str, Any]:
    ...
    clear_amount = list(chunks(clear_amount, stages * stars))
    unlock_next = list(chunks(unlock_next, stars))
...
def get_legend_quest_progress(lengths: dict[str, Any]):
    ...
    clear_amount = list(chunks(clear_amount, stages * stars))
    tries = list(chunks(tries, stages * stars))
...
def get_play_time() -> dict[str, Any]:
    ...
    frames = max(min(frames, 0), (2**31) - 1)
    seconds, frames = divmod(frames, 30)
    minutes, seconds = divmod(seconds, 60)
    hours, minutes = divmod(minutes, 60)
    play_time_data =  {"hh": hours, "mm": minutes, "ss": seconds, "frames": frames}
...
def parse_save(
    ...
    save_stats["editor_version"] = "1.1.1" #FIXME: 이거 버전은 나중에 바꿔야함
    ...
    if (
        ...
        print("Warning: DST detected is not correct for this save version, this may cause issues with save parsing.")
    ...
    save_stats["itf_timed_scores"] = list(
        chunks(get_length_data(4, 4, 51 * 3), 51)
    )
